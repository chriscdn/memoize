...
const Memoize = (cb, options = {}) => {
	...
	const memoizedFunction = (...args) => {
		...
		if (cache.has(key)) return cache.get(key);
			...
			const ttlResults = ttl({
				value: returnValue,
				key,
				args
			});
			if (isUndefined(returnValue)) {} else if (!shouldCache({
				value: returnValue,
				key,
				args
			})) {} else if (isNumber(ttlResults) && ttlResults > 0) cache.set(key, returnValue, { maxAge: ttlResults });
...
const MemoizeAsync = (cb, options = {}) => {
			...
			const promise = (async () => {
				...
				if (isUndefined(value)) return value;
					...
					if (!shouldCache({
						...
						key,
						args
					})) {} else if (isNumber(ttlResults) && ttlResults > 0) cache.set(key, value, { maxAge: ttlResults });
	...
	const memoizedFunction = async (...args) => {
		...
		if (isDefinedOrNull(cachedValue)) {
			if (isNumber(cachedTTL) && refreshWhen({
				ttl: cachedTTL,
				args,
				value: cachedValue,
				key
